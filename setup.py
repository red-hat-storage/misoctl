...
def read_module_contents():
    with open('misoctl/__init__.py') as misoctl_init:
        return misoctl_init.read()
...
class BumpCommand(Command):
    ...
    def run(self):
        ...
        module_file = read_module_contents()
        with open('misoctl/__init__.py', 'w') as fileh:
...
class ReleaseCommand(Command):
    ...
    def run(self):
        ...
        tarball = 'dist/%s-%s.tar.gz' % ('misoctl', version)
...
class PyTest(TestCommand):
    ...
    def run_tests(self):
        ...
        import pytest
        args = 'misoctl --flake8 ' + self.pytest_args
...
setup(
    name='misoctl',
    description='Koji Content Generator for Debian builds',
    packages=['misoctl'],
    author='Ken Dreyer',
    author_email='kdreyer@redhat.com',
    url='https://github.com/red-hat-storage/misoctl',
    version=metadata['version'],
    license='MIT',
    zip_safe=False,
